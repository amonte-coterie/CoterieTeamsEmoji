...
console.clear();
console.warn("Running emoji hack...");
console.log("TODO: Remove check on secret key");
if (window.SECRET_EMOJI_KEY != 'set' || true) {
...
function inject(emojiApiPath) {
    ...
    var emojiMatch = /::([\w-]+)::/g; //CHANGED to double colon
    ...
    function injectPreviewButton(previousPreviewButton, emojiList) {
        console.log("TODO: Create new icon to trigger emoji search grid instead of copying over existing.");
    ...
    // TODO: Fix the emoji selection grid.
    var CssInject = `
.emoji-img {
...
.emoji-flex-table .emoji-img {
    ...
    function init() {
        ...
        getValidEmojis().then(emojis => {
            console.log("emoji fetch response", emojis);
            var emojisUsed = {};
            setInterval(
                () => {
